    ...
    constructor(args) {
        ...
        this.runOnTargetedNodes = async (params) => {
            ...
            if (params.code) {
                ...
                const blockstore = new lit_third_party_libs_1.LitThirdPartyLibs.MemoryBlockstore();
                ...
                let lastCid;
                for await (const { cid } of lit_third_party_libs_1.LitThirdPartyLibs.importer([{ content }], blockstore, {
                    onlyHash: true,
                })) {
                    lastCid = cid;
                }
        ...
        this.throwNodeError = (res) => {
            if (res.error && res.error.errorCode) {
                ...
                (0, misc_1.throwError)({ ...res.error, name: 'NodeError' });
            }
            else {
                (0, misc_1.throwError)({
            ...
            keys.forEach((key) => {
                ...
                if (this.networkPubKeySet === null) {
                    (0, misc_1.throwError)({
                        ...
                        error: constants_1.LIT_ERROR.PARAM_NULL_ERROR,
                    });
                    return;
                }
                ...
                if (sigType !== 'BLS' && sigType !== 'ECDSA') {
                    (0, misc_1.throwError)({
                        ...
                        error: constants_1.LIT_ERROR.UNKNOWN_SIGNATURE_TYPE,
                    });
                    return;
                }
                let signature;
            ...
            Object.keys(decryptedData[0]).forEach(async (key) => {
                ...
                if (algorithmType === 'BLS') {
                    ...
                    (0, misc_1.throwError)({
                        ...
                        error: constants_1.LIT_ERROR.UNKNOWN_DECRYPTION_ALGORITHM_TYPE_ERROR,
                    });
                }
                decryptions[key] = {
        ...
        this.executeJs = async (params) => {
            ...
            if (!this.ready) {
                ...
                (0, misc_1.throwError)({
                    message,
                    error: constants_1.LIT_ERROR.LIT_NODE_CLIENT_NOT_READY_ERROR,
                });
            }
        ...
        this.getSignedChainDataToken = async (params) => {
            ...
            if (goodShares.length < this.config.minNodeCount) {
                ...
                (0, misc_1.throwError)({
                    ...
                    error: constants_1.LIT_ERROR.UNAUTHROZIED_EXCEPTION,
                });
            }
            // ========== Result ==========
        ...
        this.signSessionKey = async (params) => {
            ...
            const res = await this.handleNodePromises(nodePromises);
            // -- case: promises rejected
            if (!tslib_1.__classPrivateFieldGet(this, _NodeClient_isSuccessNodePromises, "f").call(this, res)) {
                this.throwNodeError(res);
                return {};
            }
            const responseData = res.values;
        ...
        _NodeClient_isSuccessNodePromises.set(this, (res) => {
        ...
        this.getWebAuthnAuthenticationAuthSig = async (params) => {
            // ========== Pre-Validations ==========
            // -- validate if it's ready
            if (!this.ready) {
                const message = 'LitNodeClient is not ready.  Please call await litNodeClient.connect() first.';
            ...
            const requestId = this.getRequestId();
            const nodePromises = this.getNodePromises((url) => {
                return this.getVerifyWebAuthnAuthenticationKeyShare(url, {
        ...
        this.getVerifyWebAuthnAuthenticationKeyShare = async (url, body, requestId) => {
            (0, misc_1.log)('getVerifyWebAuthnAuthenticationKeyShares');
        ...
        this.getSessionSigs = async (params) => {
            ...
            if (walletSig.address === '' ||
                ...
                (0, misc_1.throwError)({
                    ...
                    error: constants_1.LIT_ERROR.WALLET_SIGNATURE_NOT_FOUND_ERROR,
                });
                return;
            }
...
_NodeClient_isSuccessNodePromises = new WeakMap();
