...
export type DeepPartial<T> = {
    [P in keyof T]?: T[P] extends Array<infer U> ? Array<DeepPartial<U>> : T[P] extends ReadonlyArray<infer Z> ? ReadonlyArray<DeepPartial<Z>> : DeepPartial<T[P]>;
};
/**
 ...
 * ```
 */
export type XOR<T extends UnknownObject, U extends UnknownObject> = (Without<T, U> & U) | (Without<U, T> & T);
/**
 ...
 * ```
 */
export type OneOf<T, K extends keyof T = keyof T> = Prettify<Omit<T, K> & {
 ...
 */
export type Gettify<T, P extends keyof T = keyof T> = P extends string | number ? T[P] extends UnknownObject ? `${P}.${Gettify<T[P]>}` : P : never;
/**
 * Primitive types
 * @internal
 */
...
export type NonEmptyArray<T> = Overwrite<[
    ...
    map<U>(callbackfn: (value: T, index: number, array: T[]) => U, thisArg?: unknown): NonEmptyArray<U>;
}>;
/**
...
export type TwoAtLeastArray<T> = Overwrite<[
    ...
    map<U>(callbackfn: (value: T, index: number, array: T[]) => U, thisArg?: unknown): TwoAtLeastArray<U>;
}>;
/**
